...
set.seed(1030)
strain <- lapply(X, function(x) sort(sample(nrow(x), floor(2 * nrow(x) / 3))))
...
Below is the plot for all
```{r, fig.width = 8, fig.height = 8}
ggplot(data = df00, aes(x = width, y = value, col = variable)) + geom_point() + 
	scale_y_log10() + 
